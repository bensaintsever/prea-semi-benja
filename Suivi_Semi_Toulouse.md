...
**Semelles orthopédiques — sujet ouvert le 14/09.** Première mention dans ce suivi : Benjamin porte des semelles et voit son podologue **lundi 21/09** pour un point, avec la possibilité de les retirer. Aucun historique connu à ce stade (prescrites pour quoi, depuis quand, lien éventuel avec le genou ou la hanche) — questions posées à Benjamin, réponses à consigner ici. **Risque identifié** : si le retrait est décidé le 21/09, ça superpose deux variables mécaniques en cours (rodage Sonicblast démarré cette semaine + suppression semelles), alors que la règle jusqu'ici a été une variable nouvelle à la fois. À séquencer si le podologue donne le feu vert, pas à combiner. Décision finale hors de portée du plan, revient au podologue.
