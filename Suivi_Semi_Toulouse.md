...
**Illustrations** : d'abord remplacées par des schémas SVG dessinés à la main (pour tenir hors ligne et éviter toute question de droits), puis Benjamin a demandé de remettre les vraies images externes — la gestion des droits n'étant pas mon rôle ici. Images restaurées (mêmes sources qu'avant : Competitive Edge PT, RunnersConnect, Ask Doctor Jo, Wikimedia). **Point non résolu** : impossible de vérifier moi-même si ces URLs chargent réellement, la sandbox du navigateur de test bloque les requêtes externes. Le pied de page d'origine admettait déjà ne jamais avoir vérifié ces liens. À confirmer par Benjamin une fois la page ouverte en vrai ; remplacer celles qui sont mortes.
